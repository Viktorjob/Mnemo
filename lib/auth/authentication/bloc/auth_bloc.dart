...
import 'package:mnemo/auth/authentication/bloc/auth_event.dart';
  ...
  AuthBloc() : super(const AuthState.initial()) {
    on<AuthEvent>((event, emit) async {
      await event.map(
        ...
        checkEmailVerified: (e) => _checkEmailVerified(emit), // ✅ теперь будет существовать
      );
    });
  }

  Future<void> _checkEmailVerified(Emitter<AuthState> emit) async {
    final user = _auth.currentUser;
    await user?.reload(); // обновляем данные из Firebase
    if (user != null && user.emailVerified) {
      emit(AuthState.authenticated(user));
    } else {
      emit(const AuthState.error('Email not verified yet.'));
    }
  }
